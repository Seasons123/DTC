...
tableCommonFn = {
                ...
                if(tableInfo.message){
                    ...
                    if( isLeader == 1 ){
                        ...
                        for(var i=0; i<user_info.length; i++){
                            $('#member'+ (i+1)).css("display", "").html(user_info[i].name);
                        }
                    }
                    tdNum = levelNum ;
                    var indicatorArray = [];
    ...
    searchUserInfo: function (){
        ...
        $.ajax({
            ...
            crossDomain: true,
            async: false,
            success: function (taskExpertData) {
                if(taskExpertData.message){
                    $.messager.alert('错误', taskExpertData.message, 'error');
                }else{
                    for(var i in taskExpertData){
